...
Singleton {
    ...
    property int syncState: {
        if (!syncEnabled)
            return Todo.SyncState.Offline;
        return Todo.SyncState.Idle;
    }
    ...
    onTodoistApiTokenChanged: {
        ...
        if (syncEnabled && syncState !== Todo.SyncState.Syncing) {
            syncWithTodoist();
        }
        updateSyncState();
    }
    ...
    function updateSyncState() {
        if (!syncEnabled) {
            syncState = Todo.SyncState.Offline;
        } else if (syncState === Todo.SyncState.Syncing)
        // Keep syncing state
        {} else {
            syncState = Todo.SyncState.Idle;
        }
    }

    Timer {
        interval: root.syncInterval
        running: root.syncState === Todo.SyncState.Idle
        repeat: true
    ...
    function syncWithTodoist() {
        if (!syncEnabled || syncState === Todo.SyncState.Syncing)
            return;
        syncState = Todo.SyncState.Syncing;
            ...
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    try {
                        ...
                        syncState = Todo.SyncState.Error;
                    }
                } else {
    ...
    Component.onCompleted: {
        if (root.todoistApiToken.length < 5) {
            KeyringStorage.fetchKeyringData();
        }
        loadFromConfig();
        console.log(root.todoistApiToken);
        if (syncEnabled) {
            Qt.callLater(() => syncWithTodoist());
        }
    }
}
